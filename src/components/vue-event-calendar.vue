...
<script>
import { isEqualDateStr, dateTimeFormatter } from './../tools.js'
...
export default {
  ...
  data () {
    let today = dateTimeFormatter(new Date(), 'Y/M/d')
    let events = this.events.filter(function (event) {
      return isEqualDateStr(event.date, today)
    })
    ...
    calendarParams () {
      ...
      if (inBrowser) {
        ...
        return {
          ...
          curMonth: dateObj.getMonth(),
          curDate: dateObj.getDate(),
          curEventsDate: dateTimeFormatter(dateObj, 'Y/M/d')
        }
      }
    }
  },
